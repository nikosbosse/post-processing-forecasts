...
# TODO: Suggest here package
setwd("/Users/maherp/Desktop/Universitaet/Goettingen/3_Semester/Statistisches_Praktikum/post-processing-forecasts")
...
# TODO: Wrap Loop in new function that takes whole data frame as input and returns equally shaped data frame
for (h in horizons) {
  for (q in quantiles_below_median) {
    df_updated <- update_predicted_values(df = df_updated, example_model = example_model, t = target_type, h = h, q = q)
  }
}

test_df_updated(df = df, df_updated = df_updated, example_model = example_model, t = "Cases", h = 1, q = 0.025)
```
...
```{r}
scores_original <- eval_forecasts(df, summarise_by = c("model", "target_type"))
scores_updated <- eval_forecasts(df_updated, summarise_by = c("model", "target_type"))
...
```{r}
collect_predictions(original = df, cqr = df_updated) |> 
  plot_intervals(model = example_model, alpha = 0.05)
```
