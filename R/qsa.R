...
body_update_subset_qsa <- function(subset, method, cv_init_training, penalty_weight, optim_method, lower_bound_optim, upper_bound_optim, steps_optim) {
  ...
  if (is.null(cv_init_training)) {
    ...
    optimal_spread_factor <- optimize_spread_factor(
      method = method, subset = subset,
    ...
    # Run optimization to get the optimal spread factor
    optimal_spread_factor <- optimize_spread_factor(
      method = method, subset = subset_train, ########### Big typo: before we had only subset here so trained on full subset no CV!
      penalty_weight = penalty_weight, optim_method = optim_method,
      lower_bound_optim = lower_bound_optim, upper_bound_optim = upper_bound_optim, steps_optim = steps_optim,
      par = NULL
    )
...
sequence_update_subset_qsa <- function(df, method, model, location, target_type, horizon, cv_init_training, penalty_weight, optim_method, lower_bound_optim, upper_bound_optim, steps_optim) {
  # must be placed on filtered data frame (i.e. lowest level, not in
  # update_predictions()) such that fractional inputs can be correctly converted
