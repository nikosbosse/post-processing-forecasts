  ...
  for (method in c(methods)) {
    ...
    if (stringr::str_detect(method, "qsa")) {
      if (parallel) {
        ...
        df_updated <- foreach::foreach(
          m = time_series_ids["model"][[1]],
          ...
          # penalty_weight <- NULL
          updated_subset <- parallel_update_subset_qsa(
            subset, method, cv_init_training, penalty_weight, optim_method,
            lower_bound_optim, upper_bound_optim, steps_optim
          )
          updated_subset <- fix_quantile_crossing(updated_subset, m, l, t, h)
          return(updated_subset)
        }
        # As we rbind together the individual subsets we need to add the
        # cv_init_training value as attribute afterwards
