...
update_predictions(df,
  methods = c("cqr", "cqr_asymmetric", "cqr_multiplicative"), verbose = TRUE,
  cv_init_training = 0.5, model = unique(df$model)[2]
)
```
...
```{r}
# check if results are equal for cqr method after refactoring, right now they are not !!
library(dplyr)
...
```{r}
scoringutils::score(cqr3_results) |>
