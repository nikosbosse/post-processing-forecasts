...
# Compare results with old and adjusted multiplicative versions
...
old <- update_predictions(df, method, model, cv_init_training = 0.5)
old <- collect_predictions(old)

regularized_scores <- update_predictions(
  df, method, model,
  cv_init_training = 0.5, regularize_scores = TRUE
)
regularized_scores <- collect_predictions(regularized_scores)

constrained_margins <- update_predictions(
  df, method, model,
  cv_init_training = 0.5, constrain_margins = TRUE
)
constrained_margins <- collect_predictions(constrained_margins)

both <- update_predictions(
  df, method, model,
  cv_init_training = 0.5, regularize_scores = TRUE, constrain_margins = TRUE
)
both <- collect_predictions(both)
```
...
# Multiplicative Method overfits most
...
```{r}
df_eval <- cqr3_results |> eval_methods(summarise_by = "quantile")
plot_eval(df_eval)
```
