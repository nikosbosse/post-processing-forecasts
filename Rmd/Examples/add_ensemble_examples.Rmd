...
df_ensemble <- add_ensemble(df_combined, train_val_split = TRUE, verbose = TRUE, max_iter = 1e5)
...
df_ensemble |>
  ...
  # scoringutils::summarise_scores(by = c("method", "horizon", "target_type", "quantile")) |>
  dplyr::select(method:dispersion) |>
  # dplyr::arrange(quantile, horizon, target_type) |>
...
df_subset |>
  dplyr::filter(method == "cqr") |>
  scoringutils::score() |>
  dplyr::pull(interval_score) |>
  sum()
...
compute_wis(
  1,
  true_values,
  quantiles_low_matrix[, 1],
  quantiles_high_matrix[, 1]
)
```
