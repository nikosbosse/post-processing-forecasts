...
df_save <- cross_validation(
  ...
  methods = "cqr", training_lengths = c(40)
) # seq(40,73,1)
...
```{r}
# unique(df$model)
df_forecast <- update_predictions(
  df = df,
  methods = "cqr",
...
old <- dplyr::filter(df, location == "GB" & model == "epiforecasts-EpiExpert" & target_type == "Cases" & horizon == 1 & quantile == 1 - 0.05) |>
  dplyr::arrange(target_end_date) |>
  dplyr::pull(prediction)

new <- dplyr::filter(df_forecast, location == "GB" & model == "epiforecasts-EpiExpert" & target_type == "Cases" & horizon == 1 & quantile == 1 - 0.05) |>
  dplyr::arrange(target_end_date) |>
  dplyr::pull(prediction)
