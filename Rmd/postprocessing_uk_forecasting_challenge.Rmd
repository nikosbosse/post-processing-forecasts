...
df_combined <- update_predictions(df,
  ...
  collect_predictions()
# TODO: add extract_validation_set & train set (splitting function) that can be piped after collect predictions
```
...
extract_validation_set <- function(df_combined) {
  ...
  # TODO: preprocess the target_end_date column from character to Date in update_predictions. make a warning
  #       If target_end_date is a character column --> Date & give back a warning
  #       helper function dafür die helper_date heißt.
  #       makes plotting functions and cross validation much easier
  #       Damit piping funktioniert eine extract train und eine extract validation  
  #       function, beide rufen eine helper function auf da sie ja ähnlich sind
  #       Im grunde muss nur das "-" weg in [-(1:cv_init_training)]

  # based on the length of the validation set extract the dates of the validation set
  sorted_unique_target_end_dates <- sort(as.Date(na.omit(unique(df_combined$target_end_date))))
  validation_set_target_end_date <- sorted_unique_target_end_dates[-(1:cv_init_training)]
