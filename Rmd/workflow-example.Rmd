...
plot_predictions(df, model, alphas = c(0.01, 0.05, 0.25, 0.5, 0.75, 0.95, 0.99))
```
...
plot_cqr_results(df, model, t = "Cases", h = 1, q = 0.05)
```

## for given `model` and `quantile`

```{r}
plot_intervals(combined_df, model, alpha = 0.05)
```
