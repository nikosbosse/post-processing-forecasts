...
```{r}
# Option 1 (new)
df_list <- update_predictions(df, methods = "cqr", model, location, cv_init_training = 3, return_list = TRUE)
...
```{r}
# change allows for neat workflow without creating temporary variables
update_predictions(df, methods = "cqr", model, location, cv_init_training = 3, return_list = TRUE) |>
  collect_predictions() |>
...
update_predictions(df, methods = "cqr", model, location, cv_init_training = 3, return_list = TRUE) |>
  collect_predictions() |>
  plot_intervals(model, location, target_type = "Cases", quantile = 0.05, horizon = 1)
```
