...
```{r}
# Option 1 (new)
df_list <- update_predictions(df, method = "cqr", model, location, cv_init_training = 3, filter_original = TRUE)
df_combined_1 <- collect_predictions(original = df_list$original, cqr = df_list$cqr)
```

```{r}
# Option 2 (old)
df_updated <- update_predictions(df, method = "cqr", model, location, cv_init_training = 3)
