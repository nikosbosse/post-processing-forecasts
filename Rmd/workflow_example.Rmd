...
```{r}
plot_intervals(df_extended, models, locations, target_type = "Cases", quantile = 0.01, horizon = 1)
```

## for given `model`, `location` and `quantile` and different `horizons`

```{r}
plot_intervals_grid(df_extended, models, facet_by = "horizon", highlight_cv = TRUE)
```

## for given `model`, `location` and `horizon` and different `quantiles`

```{r}
plot_intervals_grid(df_extended, models, facet_by = "quantile", highlight_cv = TRUE)
```
