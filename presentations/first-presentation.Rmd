...
<!-- M: Grundsätzliches:
- Müssen entscheiden ob wir einen Punkt nach Bullet points machen. Ich findes es ohne besser.
- Ich finde wir sollten versuchen unsere Bullte points möglichst kurz zu formulieren und mehr dazu zu sagen. Dann wirkt es weniger abgelesen.
- Wenn wir lange sätze machen sollten wir zumindest die EKy wörter in den Bullte farblich und fett kennzeichnen (powerpoint best practice)
- Können wir die abstände zwishcen bullet points größer machen damit die Folien besser ausgefüllt sind?
- Wann Groß und wann Kleinschreibung in Bullet Points
...
- Part of ongoing research project by the **epiforecasts** group at the London School of Hygiene & Tropical Medicine where our project partner Nikos Bosse is engaged as a doctoral candidate
...
- **Quantile Regression Averaging**: <br>
  For given quantile level $\alpha$, new prediction is a weighted average of multiple existing predictions from different models. 
    The individual contributions, i.e. the weights, are chosen to minimize a custom loss function.
...
<!--M: Darüber würde ich gerne nochmal sprechen. Wie genau funktioniert der Intervall Score wenn wir z.B.über die Horizonte zusammenrechnen? -->
...
Note that the **same** margin is subtracted/added for the lower and upper bound, which limits flexibility.
...
```
<!-- M: ommited the comment: # ideally same interface for alternative methods . What do we want to say whith this? -->
...
- - Extension / Refinement of the CQR Method with asymmetric and/or multiplicative approach
