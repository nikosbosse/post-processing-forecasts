...
As noted in the previous section naive multiplicative Conformalized Quantile Regression without any regularization is useless for improving quantile predictions.
Typically, one can observe strong overfitting on the Training Set such that the training performance indicated promising effects, yet the scores on the validation set were *much* worse than the original forecasts.
Further, the adjusted intervals were shifted upwards and too large in most of the cases.
...
The regularizing effect of the multiplicative version can be detected immediately.
Similar to vanilla CQR, the corrected intervals are centered around the same midpoint as the original forecasts due to step $2$ of the regularization procedure of the previous section.
Because of step $1$ of the regularization and in strong contrast to the additive CQR versions, the interval *widths* are now *decreased* at most time points and appear (too) narrow in some cases.
...
Hence, we must conclude that the original CQR algorithm as described in [@romano2019] can *not* be transformed to multiplicative margins in any straightforward way. 
