...
tab1 |>
  ...
  display_table(
    caption = paste(
      "Performance of asymmetric CQR for UK",
      "and European Forecast Hub Data on Validation Set"
    )
  )
```
...
p1 <- plot_eval(uk_cqr_asym_model_quantile) +
  labs(
    y = NULL,
    title = "CQR Asymmetric Performance by Model and Quantile",
    subtitle = NULL
  ) +
  theme(
    axis.text.x = element_text(color = c(b, rep(c(t, b), 11)))
  )
...
p2 <- plot_eval(uk_cqr_asym_horizon_quantile) +
  labs(
    y = NULL,
    title = "CQR Asymmetric Performance by Horizon and Quantile",
    subtitle = NULL
  ) +
  theme(
    axis.text.x = element_text(color = c(b, rep(c(t, b), 11)))
...
```{r, hub-cqr-asymmetric-location, echo=FALSE, fig.cap="Asymmetric CQR has negative effects on almost all countries"}
...
p1 <- plot_eval(hub_cqr_asym_location_quantile) +
  labs(
    y = NULL,
    title = "CQR Asymmetric Performance by Country and Quantile",
    subtitle = NULL
  ) +
  theme(
    axis.text.x = element_text(color = c(b, rep(c(t, b), 11)))
...
p2 <- plot_eval(hub_cqr_asym_location_horizon) +
  labs(
    y = NULL,
    title = "CQR Asymmetric Performance by Country and Horizon",
    subtitle = NULL
  )
...
tab1 |>
  ...
  display_table(
    caption = paste(
      "Performance of asymmetric CQR for UK and European",
      "Forecast Hub Data for Covid-19 Cases and Deaths"
    )
  )
```
