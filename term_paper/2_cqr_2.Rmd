...
p1 <- plot_eval(uk_cqr_asym_model_quantile, base_size = 7) +
  labs(
    y = NULL,
    title = "CQR Asymmetric Performance\nby Model and Quantile",
    subtitle = NULL
  ) +
  theme(
...
```{r, hub-cqr-asymmetric-eval, echo=FALSE, fig.cap="Performance of asymmetric CQR on European Forecast Hub data"}
b <- "black"
t <- "transparent"
...
In summary, asymmetric Conformalized Quantile Regression can lead to improved prediction intervals as it is the case for the UK data set.
However, the vast majority of countries in the European Forecast Hub did not benefit from this first CQR modification.

Compared to the traditional CQR algorithm, relaxing the symmetry assumption had a negative effect on the Weighted Interval Score across both data sets.
It is worth noting that allowing for separate margins did *not* cause significant overfitting as one might assume, the original CQR algorithm outperforms the asymmetric version even on the training set.
