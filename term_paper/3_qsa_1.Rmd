...
Using the median based definition, the next step is to determine how to optimally adjust the quantile spreads. As target function, QSA uses the Weighted Interval Score \Cref{wis}. Equation \Cref{eq:qsa} shows how the QSA weights $\mathbf{w}$ influence the WIS, where $p$ denotes the number of confidence intervals, $\alpha_{p}$ the certainty level of a confidence interval and $n$ the number of observations.
...
\text{flexibel\_symmetric} &: w_i = w_{p-i} \quad i \in [0, 1, \ldots, \frac{p}{2}-1], \quad c_{i} \in \mathbb{R} \\
\text{flexible} &: w_i \in \mathbb{R} 
(\#eq:qsa-flavors)
\end{split}
\end{align}
<!--  -->
...
Underneath the hood, `postforecasts` accesses the `optim()` function from the [stats](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/optim) package. Out of the available optimization methods, `BFGS` and `L-BFGS-B` turned out to be the most reliable for QSA. 
