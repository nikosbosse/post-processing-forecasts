...
We begin by examining the results of the `qsa_uniform` method and taking a high level view. \Cref{tab:qsa-all-methods-table} presents the performance on the validation set, aggregated over all *models*, *target types*, *horizons* and *quantiles*. `qsa_uniform` clearly improves the Weighted Interval Score as it drops by `r uniform_wis_rel` percent. As expected post-processing makes the prediction intervals larger as the dispersion increases by a factor of `r uniform_dis_fac`. 

The increased intervals cover more observations and thereby reduce the under- and overprediction by `r uniform_under_pred_rel` and `r uniform_over_pred_rel`. Interestingly while both decreases are similar in terms of relative performance increases, there absolute effects on the interval score differ substantially. The underprediction reduction decreases the WIS by `r uniform_under_pred_val` which amounts to a relative decrease of merely `r uniform_under_pred_rel_wis` percent, while the overprediction drops by `r uniform_over_pred_val` which in relative terms are `r uniform_over_pred_rel_wis` percent. The main driver behind the increasing in the intervals, is that they do not reach low enough. Thus by increasing the intervals and achieving better coverage of smaller observations, while at the same time sacrificing interval sharpness, `qsa_uniform` improves the WIS.
...
df_hor_quant <- eval_methods(uk_qsa, summarise_by = "model") |>
  select(model,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
uk_qsa_human |> count(model)

df_hor_quant <- eval_methods(uk_qsa_human, summarise_by = "model") |>
  select(model,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
```{r, qsa-all-methods-models, echo=FALSE}
df_hor_quant <- eval_methods(uk_qsa, summarise_by = "target_type") |>
...
df_hor_quant <- eval_methods(uk_qsa_human, summarise_by = "target_type") |>
  select(target_type,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
uk_qsa_forecaster <- uk_qsa |>
  dplyr::filter(model %in% c("EuroCOVIDhub-baseline", "EuroCOVIDhub-ensemble")) #only human forecast

uk_qsa_forecaster |> count(model)

df_hor_quant <- eval_methods(uk_qsa_forecaster, summarise_by = "target_type") |>
  select(target_type,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
tab_original_target_type <- uk_qsa_uniform |>
  extract_validation_set() |>
  scoringutils::score() |>
  dplyr::filter(method == "original") |>
  scoringutils::summarise_scores(by = c("target_type")) |>
  dplyr::select(target_type:interval_score, dispersion, underprediction, overprediction)

target_type_col <- tab_original_target_type$target_type
tab_original_target_type <- tab_original_target_type |>
  select(-target_type)


tab_uniform_target_type <- uk_qsa_uniform |>
  extract_validation_set() |>
  scoringutils::score() |>
  dplyr::filter(method == "qsa_uniform") |>
  scoringutils::summarise_scores(by = c("target_type")) |>
...
tab_uniform_target_type_changes <- (tab_uniform_target_type - tab_original_target_type) / tab_original_target_type * 100
tab_uniform_target_type_changes$target_type <- target_type_col
tab_uniform_target_type_changes$method <- "uniform"

#Flexible Symmetric
tab_flexible_symmetric_target_type <- uk_qsa_flexible_symmetric |>
  extract_validation_set() |>
  scoringutils::score() |>
  dplyr::filter(method == "qsa_flexible_symmetric") |>
  scoringutils::summarise_scores(by = c("target_type")) |>
...
tab_flexible_symmetric_target_type_changes <- (tab_flexible_symmetric_target_type - tab_original_target_type) / tab_original_target_type * 100
tab_flexible_symmetric_target_type_changes$target_type <- target_type_col
tab_flexible_symmetric_target_type_changes$method <- "symmetric"


# Flexible
tab_flexible_target_type <- uk_qsa_flexible |>
  extract_validation_set() |>
  scoringutils::score() |>
  dplyr::filter(method == "qsa_flexible") |>
  scoringutils::summarise_scores(by = c("target_type")) |>
...
tab_flexible_target_type_changes <- (tab_flexible_target_type - tab_original_target_type) / tab_original_target_type * 100
tab_flexible_target_type_changes$target_type <- target_type_col
tab_flexible_target_type_changes$method <- "flexible"


tab_qsa_target_type_changes <- bind_rows(tab_uniform_target_type_changes, tab_flexible_symmetric_target_type_changes, tab_flexible_target_type_changes) |>
  rename(wis = interval_score, dis=dispersion, under=underprediction, over=overprediction) |>
  select(target_type, method, everything()) |>
  arrange(
    match(target_type, c("Cases", "Deaths")),
    match(method, c("uniform", "symmetric", "flexible")), 
    wis, dis, under, over)

display_table(tab_qsa_model_changes, caption="QSA Methods differ in preformance across Target Types.", bold_header = TRUE, striped = FALSE)
```

Comparing the QSA methods across target types reveals notable differences. Table \Cref{tab:qsa-all-methods-target-types} shows the relative changes in WIS after applying `qsa_uniform`, `qsa_flexible_symmetric` and `qsa_flexible` to the original data broken down by the `target_type`.

- Overall all methods help for both target types
- for cases however the gain soley comes from human forecast models
- for deaths gain comes soley from forecaster models
- We see that flexible either makes big gain or big loss
- uniform and symmetric either smaller gain or next to none, less risk


## Across Horizons

```{r, qsa-all-models-horizons, echo=FALSE, out.width="80%", fig.cap="QSA Flexible overfits as it intervals are to low in the training and to high in the validation set."}
df_hor_quant <- eval_methods(uk_qsa, summarise_by = "horizon") |>
...
df_hor_quant <- eval_methods(uk_qsa_forecaster, summarise_by = "horizon") |>
  select(horizon,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
df_hor_quant <- eval_methods(uk_qsa_human, summarise_by = "horizon") |>
  select(horizon,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
```{r, qsa-all-methods-models, echo=FALSE}
uk_qsa_forecaster <- uk_qsa |>
  dplyr::filter(model %in% c("EuroCOVIDhub-baseline", "EuroCOVIDhub-ensemble"), target_type == "Deaths") #only human forecast
...
```{r, qsa-all-methods-models, echo=FALSE}
uk_qsa_forecaster <- uk_qsa |>
  dplyr::filter(model %in% c("EuroCOVIDhub-baseline", "EuroCOVIDhub-ensemble"), target_type == "Cases") #only human forecast
...
```{r, qsa-all-methods-models, echo=FALSE}
uk_qsa_human <- uk_qsa |>
  dplyr::filter(model %in% c("epiforecasts-EpiExpert", "epiforecasts-EpiExpert_Rt", "epiforecasts-EpiExpert_direct", "seabbs"), target_type == "Deaths") #only human forecast
...
- In aggregate methods are more useful for larger horizons. So 3 and 4
- However the gains are soley for the human forecast models in the aggregate
- Gains and losses again larger for qsa flexible as for other methods
- If we, in addition to splitting by model types also split by target types we see what we saw earlier: 
  - gains for deaths and forecaster models across the board, especially strong for qsa_flexible
  - losses for cases and forecaster models across the board, especially strong for qsa_flexible
  
  - losses for deaths and human models for qsa_flexible and only minor gains for other methods
  - gains for cases and forecaster models especially strong for qsa_flexible and only for horizon 3 and 4
  
- overall gains increase for larger horizons, cut off between 2 and 3
...
```{r, qsa-all-models-horizons, echo=FALSE, out.width="80%", fig.cap="QSA Flexible overfits as it intervals are to low in the training and to high in the validation set."}
df_hor_quant <- eval_methods(uk_qsa, summarise_by = "quantile") |>
...
df_hor_quant <- eval_methods(uk_qsa_human, summarise_by = "quantile") |>
  select(quantile,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
...
df_hor_quant <- eval_methods(uk_qsa_forecaster, summarise_by = "quantile") |>
  select(quantile,qsa_uniform,qsa_flexible_symmetric,qsa_flexible)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
p1
```
...
```{r, qsa-all-methods-models, echo=FALSE}
uk_qsa_forecaster <- uk_qsa |>
  dplyr::filter(model %in% c("EuroCOVIDhub-baseline", "EuroCOVIDhub-ensemble"), target_type == "Deaths", horizon %in% c(3,4)) #only human forecast
...
```{r, qsa-flexible-symmetric-hor-quant, echo=FALSE, out.width="80%", fig.cap="QSA Flexible overfits Quantiles at short Horizons."}
df_hor_quant <- eval_methods(uk_qsa_flexible, summarise_by = c("horizon", "quantile"))
df_hor_quant <- df_hor_quant |> 
  filter(horizon > 1)
p1 <- plot_eval(df_hor_quant, base_size = 8) + ggplot2::labs(x = NULL)
...
- overall improvements are large for far out quantiles
- for human models and cases this is the case as well
- for forecaster models and deaths we see larger improvements and interestingly see qsa_flexible is useful for small intervals, apparently they need to be moved. also see qsa_flexible seems to preform well here
- further digging deeper and only looking at horizons 3,4 reveal same patterns but more imrpovement for human models and cases
- horizons 3,4 for forecaster models and deaths also show further improvements butt we see the most outer qunatiels wors for qsa_uniform and flexible
- graph explains this by missing one observations .these outlier effects distort if we have so few observations as in this case.....
- can also show that losses are at outscirts and mostely by qsa flexible!!
