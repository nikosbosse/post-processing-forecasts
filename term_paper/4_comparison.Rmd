...
As we have seen on multiple occasions throughout the previous chapters, these three factors correlate with large adjustments to the original forecasts.
For large portions of the feature space the effect sizes and the effect differences between the methods are much smaller such that \Cref{fig:ch4-intervals} is not representative but still useful to gain some basic intuition.

Except for the last observations of the time series the forecasts of the two CQR versions are quite similar and significantly closer to the original predictions than the QSA intervals. 
...
Since the ensemble method is a linear combination of the individual methods, its corrections are generally not symmetric and the intervals are centered around a different midpoint than the original forecasts.
...
As a simple weighted average with weights close to $0.5$ the lower (upper) bounds of the ensemble intervals are therefore approximately halfway between the lower (upper) bounds of the `cqr_asymmetric` and `qsa_flexible` intervals.
...
\Cref{fig:ch4-model-target} shows how the methods perform for each forecasting model and for Covid-19 Cases vs. Covid-19 Deaths.
The picture is surprisingly diverse: Along the rows of the top plot the `epiforecasts-EpiExpert_Rt` and the `seabbs` model benefit most from all methods whereas the `EuroCOVIDhub-baseline` model shows a negative effect for `cqr_symmetric` and `qsa_flexible`.
In confirmation of \Cref{tab:ch4-largest-weights} these are exactly the methods with the strongest overall influence on the ensemble and, indeed, the ensemble produces worse forecasts for the `EuroCOVIDhub-baseline` model as well.
...
The mixed nature is even more significant in the bottom plot.
Based on this impression we can not draw conclusions of Cases or Deaths benefit more from post-processing, the effects simply vary too much across different methods.
Comparing within the columns `qsa_flexible` and the ensemble are the only methods which indicate stronger benefits for Covid-19 Cases, all others perform better for Deaths.
While \Cref{fig:ch4-model-target} reveals the ensemble model as a good post-processing choice for Covid Cases, `cqr_asymmetric` and `qsa_flexible_symmetric` show promising results for Covid Deaths.
...
Considering forecast horizons and quantiles in \Cref{fig:ch4-horizon-quantile}, the similarities between the methods are now much stronger than in \Cref{fig:ch4-model-target}.
As we already know from previous chapters, all methods work better for larger horizons and quantiles in the tails.
The ensemble predictions are again closest to `qsa_flexible` and lead to the largest performance gains for three and four week-ahead predictions while simultaneously indicating the worst results for short-term one week ahead forecasts.
The classical `cqr` method is the only method that reliably works for any forecast horizon, although the effect sizes are quite small.

The bottom plot across different quantile levels is interesting as well: `qsa_flexible_symmetric` and `qsa_uniform` have the largest positive effect for very small or very large quantiles, while the performance drops off rapidly towards the center.
...
The asymmetric CQR version is the only method which has a negative effect and should thus be avoided for post-processing forecasts of centered quantiles. 
...
It thus offers a good choice when upsides and downsides are equally weighted in the user's cost function.
