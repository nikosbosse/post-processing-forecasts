...
$$
We define a function that returns WIS for one intervall so the IS actually. It can be used to check what the correct adjustment is.

```{r}
wis_calc <- function(x_vec = c(-20:20),
                     qs = c(0.25, 0.5, 0.75),
  ...
  for (x in x_vec){
    ...
    wis_val <- abs(u-l) #length(true_values_new) * 
  ...
  for (x in x_vec){
    subset_generated <- subset_generated |>
      dplyr::mutate(prediction = replace(
        ...
        values = rep(qs[1] * 100 - (50-qs[1]*100) * (x-1),length(unique(subset$target_end_date)))
      ))
    subset_generated <- subset_generated |>
      dplyr::mutate(prediction = replace(
        .data$prediction, .data$quantile == qs[3],
        values = rep(qs[3] * 100 + (50-qs[1]*100) * (x-1),length(unique(subset$target_end_date)))
      ))
...
- Expectation: QSA should make intervals smaller. Current WIS is 525, by reducing top and bottom intervall by 10 WIS is reduced to 375.
- Observation: Intervals remain unchanged.
...
```{r}
# Proof that implemented line search works
test(qs = c(0.25, 0.5, 0.75),
     error_factor = 1,
     true_values_new = c(rep(50,4), 60, rep(20,5)),
     methods = "qsa_uniform",
...
Expectation: QSA should make intervals smaller as it has 3 values but only need 2 in the 20% Intervall
Observation: makes intervals smaller, as both points are on the same line intervall colapses
...
ggplot() +
  geom_line(data = df, aes(x, y), colour = "black")
```
...
QSA with the BFGS method doesnt work as expected. We see that it does adjust to encompass enough observations into the intervall. However it doesnt adjust the intervals to be smaller or larger as to minimize the distance to points penalty. 

# Testing QSA Uniform with different Optimizers

## Test 3: See if other optimizing methods, available in optim are able to adjust intervals even though the number of covered observations is correct?
...
Observation: Doesnt really work but makes a slight correct adjustment.
...
Observation: It works the best. Reduces the intervals as it should. However it gives back very different results for different bounds. Furthermore the smaller bounds are the faster it is. 
...
Use the Brent method, however the setting of the bounds is key. It might make sense to run multiple optimisations with a set of start values each time and then take the one where results are best.
...
test(qs = c(0.25, 0.5, 0.75),
     ...
     quantile_plot = 0.25,
     optim_method = "Brent",
     optim_multiple_bounds_brent = TRUE
     )
```
